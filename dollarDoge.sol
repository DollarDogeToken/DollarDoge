...
contract DollarDogeDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("DollarDogeDividendTracker", "DollarDoge_DividendTracker") {
        claimWait = 600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "DollarDogeDividendTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "DollarDogeDividendTracker: withdrawDividend disabled. Use the 'claim' function on the main DollarDoge contract.");
    }
...
contract DollarDoge is ERC20, Ownable {
    ...
    DollarDogeDividendTracker public dividendTracker;
    ...
    constructor() ERC20("DollarDoge", "DollarDoge") {
        dividendTracker = new DollarDogeDividendTracker();
    ...
    function updatePancakeRouter(address newAddress) public onlyOwner {
        require(newAddress != address(pancakeRouter), "DollarDoge: The router already has that address");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "DollarDoge: The dividend tracker already has that address");

        DollarDogeDividendTracker newDividendTracker = DollarDogeDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "DollarDoge: The new dividend tracker must be owned by the DollarDoge token contract");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "DollarDoge: Account is already the value of 'excluded'");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "DollarDoge: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "DollarDoge: Cannot update gasForProcessing to same value");
